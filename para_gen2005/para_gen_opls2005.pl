...
$para_script = "/pubhome/qyfu02/zhouyu_rescore/para_gen2005";
$user = "qyfu02";
########################################################################################
...
foreach $id (@dir)
{
    ...
    print OUT "mkdir /pubdata/$user\n";
    print OUT "rm -r /pubdata/$user/$name-$id\n";
    print OUT "cp -r $output/$name/mol_tmp/$id /pubdata/$user/$name-$id\n";
    print OUT "$para_script/mol_convert.pl /pubdata/$user/$name-$id\n";
    print OUT "mv /pubdata/$user/$name-$id $output/$name/decoys/$id\n";
#    print OUT "rm -r $output/$name/mol_tmp/$id\n";	# could uncomment it to delete temporary directory
}
